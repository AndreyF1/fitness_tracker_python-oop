class InfoMessage:
    ...
    def get_message(self) -> str:
        dur = '%.3f' % self.duration
        dis = '%.3f' % self.distance
        sp = '%.3f' % self.speed
        cal = '%.3f' % self.calories
        info = (f'Тип тренировки: {self.training_type}; '
                f'Длительность: {dur} ч.; '
                f'Дистанция: {dis} км; '
                f'Ср. скорость: {sp} км/ч; '
                f'Потрачено ккал: {cal}.')
        return info
...
def read_package(workout_type: str, data: list) -> Training:
    ...
    kind_of_training = training_dict.get(workout_type)(*data)
    return kind_of_training
